...
class SlidesControlPrevHandler(RequestHandler):
  """Any HTTP request to /slides-control shall increment the current slide
  number and notify all listener."""
...
class SlidesControlJumpHandler(RequestHandler):
  """Any HTTP request to /slides-control shall increment the current slide
  number and notify all listener."""
  ...
  def get(self):
    global current_slide
    try:
        current_slide = int(self.request.query)
    except ValueError:
        pass
    print('Slide updated: %d' % current_slide)
    for listener in slide_change_listeners:
...
def main():
  ...
  application = Application([
    (r"/next", SlidesControlHandler),
    (r"/prev", SlidesControlPrevHandler),
    (r"/jump", SlidesControlJumpHandler),
